...
<div class="WSY_content" id="WSY_content_height">
	...
	<?php
		...
		$query_count="select count(1) as tcount FROM weixin_commonshop_products WHERE isvalid=true AND customer_id=".$customer_id." and yundian_id < 0 and isout = 1";



	    $query2="SELECT id,name,asort_value,type_id,type_ids,orgin_price,now_price,for_price,cost_price,default_imgurl,isnew,createtime,isout,ishot,isnew,isvp,is_virtual,good_level,meu_level,bad_level,is_supply_id,create_type,sell_count,is_QR,storenum,tax_type ,privilege_level,ordering_retail,is_mini_mshop
			FROM weixin_commonshop_products WHERE isvalid=true AND customer_id=".$customer_id." and isout = 1 and yundian_id < 0 ";
		$query3="";
    ...
    <div class="WSY_data">
		...
		<form class="search" action="store.php?customer_id=<?php echo $customer_id_en; ?>">
			...
			<div class="WSY_search_div">
				<ul>
					...
					<?php if($product_num ==-1  || $product_num>$num){?>
						<li class="WSY_bottonliss left"><input type="button" style="width:100px" id="mul_onsale" value="批量上架"></li>
					<?php }?>
					<li class="WSY_bottonliss left"><input type="button" style="width:100px" id="" onclick="delete_ids();" value="批量删除"></li>
		   ...
		   <div id="div_check_store" class="div_op" style="display:none;height:auto;margin-left:20px">
			<form  id="frm_import" action="../../../excel/import_excel_store.php?customer_id=<?php echo $customer_id_en; ?>&frompage=store" enctype="multipart/form-data" method="post" class="store">
		...
		while ($row2 = mysql_fetch_object($result2)) {
			...
			$privilege_level   = $row2->privilege_level;
	  ...
	  ?>
              ...
              <tr id="WSY_q1">
                <td><input type="checkbox" name="pro_ids" id="pro_ids" value="<?php echo $p_id; ?>"></td>
				<td id="pro_<?php echo $p_id; ?>"><?php echo $p_id;?>
					<?php if($supply_id > 0){?>
                ...
                <td><span id="proname_<?php echo $p_id;?>" data-proname="<?php echo $p_name?>"><?php echo $p_name; if($is_QR == 1){ echo ' (券)';} ?></span>
					<img id="saveimg_<?php echo $p_id;?>" src="../../../common/images_V6.0/operating_icon/icon53.png" class="ep_img" onclick="toEditName('<?php echo $p_id;?>')">
				</td>
                <td><span id="protype_<?php echo $p_id;?>"><?php echo $typename; ?></span>
					<img id="savetypeimg_<?php echo $p_id;?>" src="../../../common/images_V6.0/operating_icon/icon53.png" class=" ep_img pro_typeimg"
						data-pro-typeid="<?php echo $type_ids;?>" data-pro-id="<?php echo $p_id;?>">
				</td>

				<td>
					<div class="WSY_pricebox" style="display: inline-block;">

						<li class="WSY_price01" id="prooprice_<?php echo $p_id;?>" >￥<?php echo $p_orgin_price; ?></li>
						<li data-forprice="<?php echo $p_for_price; ?>" data-costprice="<?php echo $p_cost_price; ?>" data-supplyid="<?php echo $supply_id; ?>" id="pronprice_<?php echo $p_id;?>" >￥<?php echo $p_now_price; ?></li>
					</div>
                ...
                <td><img src="<?php echo "//".$new_baseurl.$imgurl; ?>" class="WSY_fixed"  /></td>
				<td><span id="proattr_<?php echo $p_id;?>" style="float:left;padding:10px 0;"><?php echo $otherstr; ?></span>
					<img id="saveattrimg_<?php echo $p_id;?>" src="../../../common/images_V6.0/operating_icon/icon53.png"
						...
						data-ishot="<?php echo $p_ishot;?>" data-isnew="<?php echo $p_isnew;?>" data-isvp="<?php echo $p_isvp;?>" data-is_virtual="<?php echo $p_is_virtual;?>" data-privilege_level="<?php echo $privilege_level;?>" data-is_mini_mshop="<?php echo $is_mini_mshop;?>" >
				</td>
				...
				<?php if($storenum > 0 && ($product_num ==-1  || $product_num>$num)){?>
				<a title="上架" href="#" onclick="return checkonsale(<?php echo $p_id; ?>)"><img src="../../../common/images_V6.0/operating_icon/icon40.png"></a>
                <?php
				}
				}
			}?>
...
function checkonsale(kid){
	if(confirm("是否确定将选中的商品上架？")){
		var ponsale = kid;
		console.log(ponsale);
...
function delete_ids(){
	var del_arr = '';
	var arr = [];
	$("input[name='pro_ids']:checkbox").each(function(i){
		if($(this).is(':checked')){
			arr.push($(this).val());
			del_arr += $(this).val()+",";
		}
	...
	if(del_arr.length == 0){
		...
		if(confirm("确定要删除已选中的"+arr.length+"个产品吗？")){
			$.ajax({
				url:'./public/delete_arr.php',
				type:"post",
				dateType:"json",
				data:{"delete_arr":"delete_arr",delete_ids:del_arr},
...
<script type="text/javascript">
	...
	function jumppage() {
		...
		if ((a < 1) || (a == pagenum) || (a > page) || isNaN(a)) {
			...
			var supply_id = $("#supply_id").val();
			var foreign_mark = $("#foreign_mark").val();
			document.location = "store.php?customer_id=" + customer_id_en + "&foreign_mark=" + foreign_mark + "&sales=" + ordersale + "&pagenum=" + a + "&search_type_id=" + search_type_id
				+ "&search_status=" + search_status + "&keyword=" + keyword + "&search_other_id=" + search_other_id + "&search_status=" + search_status
				+ "&search_source=" + search_source + "&search_supply=" + search_supply + "&supply_id=" + supply_id;
		}
	}
...
<script type="text/javascript" src="../../Common/js/Product/product/store.js"></script>
