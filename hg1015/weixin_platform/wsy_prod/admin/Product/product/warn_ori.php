...
<div class="WSY_content" id="WSY_content_height">
	...
	<?php
		...
		$stock_remind = 1; //获取设置的警界值
		$query_remind ="select stock_remind from weixin_commonshops where isvalid=true and customer_id=".$customer_id." limit 0,1";
		$result_remind = _mysql_query($query_remind) or die("query_remind error : ".mysql_error());
		while($row_remind = mysql_fetch_object($result_remind)){
			$stock_remind = $row_remind -> stock_remind;
		}
		//查找属性商品超过警界值的
		$query_child = "SELECT distinct product_id FROM weixin_commonshop_product_prices p , ".WSY_PROD.".weixin_commonshop_products pro
			where pro.id = p.product_id and pro.customer_id = ".$customer_id." and pro.isvalid = true and pro.yundian_id < 0 and pro.isout = 0 and p.storenum <= ".$stock_remind." and pro.storenum >= ".$stock_remind;
		$result_child = _mysql_query($query_child) or die("L225 query error : ".mysql_error());
		$array_child = array();
		while($row_c = mysql_fetch_object($result_child)){
			$array_child[] = $row_c -> product_id;
		}
		...
		$query_count="select count(1) as tcount FROM weixin_commonshop_products WHERE isvalid=true AND customer_id=".$customer_id."
				and ( storenum < ".$stock_remind."".(empty($pro_str) ? "" : " or id in (".$pro_str.")").") and isout = 0 and yundian_id < 0 ";

	    $query2="SELECT id,name,asort_value,type_id,type_ids,orgin_price,now_price,default_imgurl,isnew,createtime,isout,ishot,isnew,good_level,meu_level,bad_level,is_supply_id,create_type,sell_count,is_QR ,storenum,tax_type,ordering_retail,is_mini_mshop,nowprice_title
			FROM weixin_commonshop_products WHERE isvalid=true AND customer_id=".$customer_id." and ( storenum < ".$stock_remind."".(empty($pro_str) ? "" : " or id in (".$pro_str.")").") and isout=0 and yundian_id < 0 ";
		$query3="";
