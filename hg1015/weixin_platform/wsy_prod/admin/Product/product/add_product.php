...
$product_vip_price=0;//商品VIP价 默认值0---2018-01-05-pine
$product_type_id=-1;
$product_introduce="";
$product_description="";
$specifications="";
...
if(!empty($_GET["product_id"])){
  ...
  // echo $query;
   ...
   while ($row = mysql_fetch_object($result)) {
      ...
      $product_vip_price           = $row->vip_price;
...
<?php
$proids = '';
$query="select pp.proids,pp.orgin_price,pp.now_price,pp.vip_price,pp.storenum,pp.need_score,pp.cost_price,pp.foreign_mark,pp.unit,pp.weight,pp.for_price,cp.name
...
while ($row = mysql_fetch_object($result)) {
    ...
    ?>
      ...
      var vip_price = '<?php echo $vip_price; ?>';
      ...
      ordering_vip_price += ','+"<?php echo $vip_price; ?>";


      old_name.push(name)
      ppriceHash.add(proids,orgin_price+"_"+now_price+"_"+vip_price+"_"+storenum+"_"+need_score+"_"+cost_price+"_"+foreign_mark+"_"+unit+"_"+weight+"_"+for_price);
      if(proids!=""){
          // console.log(proids,'-----')
    ...
    if(ordering_now_price != ''){
      ...
      if(ordering_vip_price != ''){
        ordering_vip_price = ordering_vip_price.substring(1)
    }
</script>
<style type="text/css">
    .type_detail{
                ...
                <div class="product-tab-list <?php if($sa == 3) echo 'tab-active'; ?>">
                    <div id="product-tab-ex">
                      <dl class="WSY_bulkbox w90px product-input-box product-input-list">
                        ...
                        <dd class='vip_price'>
                            <?php if(OOF_P != 2) echo OOF_S ?>
                            <input type="text" name="vip_price" value="<?php echo $product_vip_price; ?>" class="form_input num_check calc_np" size="5" maxlength="10">
                            <?php if(OOF_P == 2) echo OOF_S ?>
                        </dd>
...
<script>
    ...
    var vip_price_b = $("input[name='vip_price']").val();
    ...
    function first_step(obj){
        var parent          = $(obj).parents('.tab-active'),
            ...
            pro_vip_price   = parent.find('input[name="vip_price"]'),
        ...
        if(pro_vip_price.val() === ''){
            input_tip('请输入产品VIP价！',pro_vip_price);
            return false;
        }
        if(pro_weight.val() === ''){
    ...
    function getAppendText(str,pid,text){
        ...
        if(ppriceHash.contains(pid)){
            ...
            vip_price = onprices[2];
            storenum = onprices[3];
            need_score = onprices[4];
            cost_price = onprices[5];
            foreign_mark = onprices[6];
            unit = onprices[7];
            weight = onprices[8];
            for_price = onprices[9];
        }
        if(pid!=""){
            ...
            str += '<td class="vip_price"><?php if(OOF_P != 2) echo OOF_S ?><input type=\"text\" name=\"pro_vip_price\" value=\"'+vip_price+'\" class=\"form_input num_check calc_np\" size=\"5\" maxlength=\"10\"><?php if(OOF_P == 2) echo OOF_S ?></td>';
            ...
            str += '<td class="orgin_price"><?php if(OOF_P != 2) echo OOF_S ?><input type=\"text\" name=\"pro_orgin_price\" value=\"'+orgin_price+'\" class=\"form_input num_check \" size=\"5\" maxlength=\"10\"><?php if(OOF_P == 2) echo OOF_S ?></td>';
            str += '<td class="now_price"><?php if(OOF_P != 2) echo OOF_S ?><input type=\"text\" name=\"pro_now_price\" value=\"'+now_price+'\" class=\"form_input num_check calc_np\" size=\"5\" maxlength=\"10\"><?php if(OOF_P == 2) echo OOF_S ?></td>';
            str += '<td class="vip_price"><?php if(OOF_P != 2) echo OOF_S ?><input type=\"text\" name=\"pro_vip_price\" value=\"'+now_price+'\" class=\"form_input num_check calc_np\" size=\"5\" maxlength=\"10\"><?php if(OOF_P == 2) echo OOF_S ?></td>';
            str += '<td class="for_price"><?php if(OOF_P != 2) echo OOF_S ?><input type=\"text\" name=\"pro_for_price\" value=\"'+for_price+'\" class=\"form_input num_check calc_fp\" size=\"5\" maxlength=\"10\"><?php if(OOF_P == 2) echo OOF_S ?></td>';
            str += '<td class="base_price"><?php if(OOF_P != 2) echo OOF_S ?><input type=\"text\" name=\"pro_cost_price\" value=\"'+cost_price+'\" class=\"form_input num_check calc_bp\" size=\"5\" maxlength=\"10\"><?php if(OOF_P == 2) echo OOF_S ?></td>';
            //str += '<td><input type=\"text\" name=\"pro_unit\" value=\"'+unit+'\" class=\"form_input\" size=\"5\" maxlength=\"10\"></td>';
            str += '<td><input type=\"text\" name=\"pro_weight\" value=\"'+weight+'\" class=\"form_input num_check \" size=\"5\" maxlength=\"10\"></td>';
