...
$act_type 	= $configutil->splash_new($_POST["act_type"]);	//活动类型
$act_id 	= $configutil->splash_new($_POST["act_id"]);	//活动id
$pro_yundian_id 	= $configutil->splash_new($_POST["pro_yundian_id"]);	//产品所属的云店id

    /**************pine*********************/
    $info = new my_data();
 	$checkIsPromoter = $info -> checkIsPromoter($user_id);
	/**************pine*********************/


//----参数区域 end
_file_put_contents("ceshi_233.txt", "//查看pos的参数=".$pos."\r\n", FILE_APPEND);
...
while ($row = mysql_fetch_object($result)) {
	...
	/****************pine*************************/
	if($checkIsPromoter==1){
		$p_now_price	= $row->vip_price;	
	}else{
...
$query2  = "select is_identity from weixin_commonshops where isvalid=true and customer_id='".$customer_id."'";
$re_iden = _mysql_query($query2);
while ($row = mysql_fetch_object($re_iden))
{
...
if( $wholesale_id>0 ){
	//取出该订单选择的批发属性
	$pos_arr = [];
	$pos_arr = explode("_", $pos);  //属性字符转数组
	$pos_num = count($pos_arr);		//得到有多少个属性

	if( $pos_num>1 ){				//如果属性大于1种，则数组最后一个为选择的批发属性，如果只有一种，则直接为批发属性
		$wholesale_pos = end($pos_arr);
	}else{
		$wholesale_pos = $pos;
	}

	$query = "SELECT wholesale_num FROM weixin_commonshop_pros WHERE isvalid=true AND id='$wholesale_pos'";
	$result= _mysql_query($query)or die('Query failed 82: ' . mysql_error());
	while( $row = mysql_fetch_object($result) ){
		$wholesale_num = $row->wholesale_num;//最低批发数量
	}
}
/*---查询该产品是否拥有批发属性---*/
...
if( !empty( $pos ) ){
	...
	$p_storenum = 0;
	 // _file_put_contents("ceshi_233.txt", "//查询出库存的sql=".$query."\r\n", FILE_APPEND);
	while ($row = mysql_fetch_object($result)) {
		$need_score		= $row->need_score;
		$p_storenum		= $row->storenum;
		//$p_now_price	= $row->now_price;
...
switch ($act_type) {
	case 1:		//积分兑换产品
		...
		$integral = new model_integral();
		$resultArr['act_pro_info'] = $integral->cal_product_integral_per($param);
		
		if ($resultArr['act_pro_info']['errcode'] == 0 && $resultArr['act_pro_info']['data'][$pid]['isvalid'] == 1) {
			$p_now_price = $resultArr['act_pro_info']['data'][$pid]['money_p'];
		}
	break;
	case 31:		//限购活动
		$param = array(
		...
		$resultArr['act_pro_info'] = $restricted_purchase->findRestrictedPurchase($param);
		
		if ($resultArr['act_pro_info']['errcode'] == 0 && !empty($resultArr['act_pro_info']['data'])) {
			$p_now_price = $resultArr['act_pro_info']['data']['price'];
		}
	break;
	default:
		$resultArr['act_pro_info'] = array('errcode'=>-1, 'errmsg'=>'');
	break;
}
/* 按活动类型查询产品信息 end */


/*按照产品所属的云店ID查找云店信息  start */
$yundian_status = 0;
$yundian_expire_time = '';
if($pro_yundian_id != -1 && !empty($pro_yundian_id)){
	$query = "select user_id,status,expire_time from ".WSY_USER.".weixin_yundian_keeper where customer_id=".$customer_id." and isvalid=1 and id=".$pro_yundian_id;
	$result_yundian = _mysql_query($query) or die('Query failed5: ' . mysql_error());

	while ($row_yundian = mysql_fetch_object($result_yundian)) {
		$yundian_status			= $row_yundian->status;
		$yundian_expire_time	= $row_yundian->expire_time;
	}
}
/*按照产品所属的云店ID查找云店信息  end */
...
if(!empty($error)){
	...
	/*云店信息*/
	$resultArr["yundian_status"] 	  = $yundian_status;
	$resultArr["yundian_expire_time"] = $yundian_expire_time;
	/*云店信息*/
}
mysql_close($link);
$jsons=json_encode($resultArr);
