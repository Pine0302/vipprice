...
#4. 获取产品详情
    ...
    foreach ($result as $row) {
        ...
        /********************/
        $p_now_price_ori = $row->now_price;
        $p_vip_price = $row->vip_price;
    ...
    //hzq
    $now_prices = $p_now_price_ori;                         //用于防止限购活动改价
...
#5. 获取产品相关活动
    ...
    $show_now_price          = $result["show_now_price"] ?: $p_now_price_ori;
...
#6. 获取产品相关优惠券、用户积分
    ...
    $data = array(
        ...
        "o_shop_id" => $o_shop_id,
        /***********pine*************/
        'checkIsPromoter'=>$checkIsPromoter
        /***********pine*************/
    );
...
#7. 其它
    ...
    class product_detail_model{
        ...
        public function product_data($data){
            ...
            $result = [];
            $query = "select proids,orgin_price,now_price,vip_price,storenum,need_score,weight,unit from weixin_commonshop_product_prices where product_id='{$this->pid}'";
            $result_proids = mysql_select($query);
            foreach ($result_proids as $k => $v) {
                $proids = $v["proids"] ?: "";
                $orgin_price = $v["orgin_price"] ?: 0;
                $now_price = $v["now_price"] ?: 0;
                $vip_price = $v["vip_price"] ?: 0;
    ...
    //            $proids_str = substr($proids_str, 1);
                /***********************pine*******************************/
   				if($data['checkIsPromoter']==1){
                	$info =  "{$orgin_price}_{$vip_price}_{$storenum}_{$need_score}_{$unit}_{$pro_weight}";
                }else{
                ...
                /*$result[] = array(
                    "proids"    => $proids_str,
                    "info"      => "{$orgin_price}_{$now_price}_{$storenum}_{$need_score}_{$unit}_{$pro_weight}_{$vip_price}",
                    'old_proids'=> $proids
                );*/
            }
