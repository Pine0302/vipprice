...
class create_shop_order
{
    ...
    protected $qr_Utlity;
    protected $HyOrder_reward = null; //订单奖励
    ...
    public function __construct($customer_id, $user_id, $current_time) {
		...
		if(file_exists($_SERVER['DOCUMENT_ROOT'] . '/mshop/web/controller/HyOrder_reward.php') ){
			require_once($_SERVER['DOCUMENT_ROOT'] . '/mshop/web/controller/HyOrder_reward.php');
			$this->HyOrder_reward = new control_HyOrder_reward($this->customer_id);
		}
    ...
    public function create_shop_order($orderparam, $shop_setting, $user_data) {
        ...
        foreach($order_data as $order_num => $supply_order) {       //  supply_order=>商品信息
            ...
            foreach($product_info as $product_num => $product_data) {
                ...
                if(($user_data['promoter_id']>0)&&($user_data['term_of_validity']>$now_date)){
                    $prod_info['now_price'] = $prod_info['vip_price'];
                }
                /*************************pine*******************************/
                ...
                if($is_exchange_goods && $product_yundian_id == -1) {
                    ...
                    if(!empty($prod_prosid)) {
                        $prod_pros_info = $this->get_product_pros_info($prod_id, $prod_prosid);
                          file_put_contents("/opt/www/weixin_platform/test.txt", '--now_price--获取属性产品2--'.json_encode($prod_pros_info).PHP_EOL, FILE_APPEND);
                      
                        file_put_contents("/opt/www/weixin_platform/test.txt", '--now_price--获取属性产品2--extract--'.json_encode($now_price).PHP_EOL, FILE_APPEND);
                      /*************************pine*******************************/
                      $now_date = date("Y-m-d H:i:s");
                        if(($user_data['promoter_id']>0)&&($user_data['term_of_validity']>$now_date)){
                            $prod_pros_info['now_price'] = $prod_pros_info['vip_price'];
                        }
                        /*************************pine*******************************/
                    extract($prod_pros_info);
                      //  file_put_contents("/opt/www/weixin_platform/test.txt", json_encode($prod_pros_info).PHP_EOL, FILE_APPEND);
                        //判断活动产品是否已下架
                ...
                $product_act_info = $this->product_act($product_act_arr,$prod_num);
            ...
            if(!empty($opi_array) && $supply_yundian_id == -1) {//云店自营产品不适用抵购，购物币，优惠券等，仅计算原来商品的金额
                //获取供应商产品的的最优快递
			...
			/*订单奖励 by hy*/
			if($this->HyOrder_reward){
				$this->HyOrder_reward->busses_add($batchcode,$this->user_id);
			}
			/*-------订单奖励 end-------*/
    ...
    //活动产品
    public function product_act($product_act_arr,$prod_num) {
        ...
        switch($act_type) {
            case 21:    //积分赠送产品
                $result = $this->integral_give($product_act_arr,$prod_num);
                break;
    ...
    //商城积分赠送产品
    public function integral_give($product_act_arr,$prod_num) {
        extract($product_act_arr);
