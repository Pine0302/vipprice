...
include_once($_SERVER['DOCUMENT_ROOT']."/wsy_pay/sdk/IndustryOpeInstance.php");
...
$batchcode_arr=explode("_",$batchcode_str);
...
//$query_ordering = "SELECT receive_mode FROM orderingretail_setting WHERE isvalid=true AND customer_id=".$customer_id;
...
//$query_isopen_account = "SELECT isopen_account FROM orderingretail_setting WHERE isvalid=true AND customer_id=".$customer_id;
...
//获取各行业支付后返回页面
//$industry_obj = IndustryOpeInstance::getIndustryObject($industry_type , $customer_id);
//$data['batchcode'] = $batchcode_arr[0];
//$returnpage = $industry_obj->getReturnPage($data);
//var_dump($returnpage);
//echo $_SESSION['pass_url_3243'];
?>
...
//    var ordering_isopen_account="<?php //echo $ordering_isopen_account; ?>//"; //订货系统货款管理开关
    ...
    if(price == 0){
        ...
        toInternalPay("deductible");
    }else{
        $.ajax({
            url: "/weixinpl/mshop/ajax_show_payway.php",
            dataType: 'json',
            type: 'post',
            data:{'customer_id':customer_id_en,'industry_type':industry_type,"page_port":page_port,"pay_user_id":pay_user_id,"opt":opt,"order_batchcode":order_batchcode,"is_virtual":is_virtual},
                    ...
                    if (result.datalist.length == 0) {
                        ...
                        content += '<img class="pic" style="width: 80%;display: block;margin: 60px auto 0 auto;" src="./images/pic.png"><p style="text-align: center;font-size: 15px;color: #888" class="text">商家未开启任何支付方式</p>';
                            ...
                            for (i = 0; i < result.datalist.length; i++) {
                                ...
                                $balance = 0;
                                $show_balance = false;
                                if($pay_type == 'card' || $pay_type == "moneybag" || $pay_type == "account"){
                                    $show_balance = true;
                                ...
                                if($pay_type=="IPSpay" && price<2){
                                    ...
                                    if($pay_type=="virtual"){
                                        if(result.datalist[i].is_allow_virtual==1){
                                            content += '<div class = "new_popup-menu-row" data-value="' + $pay_type + '" onclick="new_popup(this,'+JSON.stringify(result.datalist[i]).replace(/"/g, '&quot;')+','+JSON.stringify(extra_info).replace(/"/g, '&quot;')+');">';
                                        }else{
                                            content += '<div class = "new_popup-menu-row" data-value="' + $pay_type + '">';
                                        }
                                    }else{
...
//                                }
    ...
    function new_popup(obj,paytype_obj,extra_obj){console.log('第一步');
        var pay_type = $(obj).data("value");
        history_replaceState(industry_type);
        toPayPage(pay_type,paytype_obj,extra_obj);
    }
    ...
    function toInternalPay(pay_type){
        ...
        $.ajax({
            ...
            dataType: 'json',
            type: 'post',
            data:{'customer_id_en':customer_id_en,'industry_type':industry_type,"order_batchcode":order_batchcode,"pay_price":price,"pay_type":pay_type,"user_id":pay_user_id,"is_merge":is_merge,"opt":opt},  //supplier_id:supplier_id,:ordering_receive_mode},
            success:function(result){
                ...
                $.ajax({
                    ...
                    type: 'post',
                    async : false,
                    data:{'customer_id_en':customer_id_en,'industry_type':industry_type,"order_batchcode":order_batchcode,"internal_opt":"get_returnpage","return_batchcode":result.batchcode,"city_type":city_type,"cash_o2o_type":cash_o2o_type,"orderingretail_opt":opt,"return_status":result.status,"supplier_id":supplier_id},
                    success:function(data){
                        history_return_page = protocol_http_host+data.history_return_page;
                        return_page = protocol_http_host+data.return_page;
    ...
    var pay_type_obj = {};
    var extra_info_obj = {};
    function toPayPage(pay_type,paytype_obj,extra_obj){console.log('第三步');
        pay_type_obj = paytype_obj;
        extra_info_obj = extra_obj;
        /* 如果是拼团订单，支付前先检测活动有效性 start */
        if( extra_obj.is_collage_order == 1 ){
            var is_go_on = true;
            $.ajax({
                ...
                data: {
                    user_id : pay_user_id,
                    pid : extra_obj.collage_info.pid,
                    num : extra_obj.collage_info.collage_rcount,
                    activitie_id : extra_obj.collage_info.activitie_id,
                    group_id : extra_obj.collage_info.group_id,
                    comefrom : 2,
        ...
        if(pay_type == 'anotherpay'){
             ...
             * */
            if( paytype_obj.is_check==1 ){  //如果需要输入支付密码
                $.ajax({
                    ...
                    success:function(result){
                        ...
                        if( result.status == 1) {
                            if(pay_type =='moneybag') {
                                ...
                                $.get('/weixinpl/mshop/calc_moneybagpay_poundage.php', {
                                    ...
                                    if (res.isopen_poundage == 1) {
                                        ...
                                        newShowConfirmMsg('零钱支付确认', html, '取消', '确定', function () {
                                            if (is_check_password == 1) {
                                                //toInternalPay(pay_type);
                                                StartPay(paytype_obj,extra_obj);
                                                loading(100, 1);
                                            } else {
     ...
     * paytype_obj : 支付信息对象
     * */
    function StartPay(paytype_obj,extra_obj){
        //内部支付
        if(paytype_obj.pay_type == "moneybag" || paytype_obj.pay_type == "card" || paytype_obj.pay_type == "virtual" || paytype_obj.pay_type == "nopay" || paytype_obj.pay_type == "account"){
            toInternalPay(paytype_obj.pay_type);
            loading(100,1);
        }else{

            var url="";
            if( paytype_obj.pay_type =='weipay' ){//微信支付
                //var pay_code = get_paycode(pay_user_id,industry_type);

                if("h5"==page_port || "app"==page_port){
                    url=protocol_http_host+paytype_obj.pay_url.h5+"?order_id="+order_batchcode+'&customer_id='+customer_id_en+'&industry_type='+industry_type+'&is_merge='+is_merge+'&city_type='+city_type;
                }else{
                    url=protocol_http_host+paytype_obj.pay_url.common+"?order_id="+order_batchcode+'&customer_id='+customer_id_en+'&industry_type='+industry_type+'&is_merge='+is_merge+'&city_type='+city_type;
                }
            }else if(paytype_obj.pay_type =='alipay'){
                url=protocol_http_host+paytype_obj.pay_url.h5+"?order_id="+order_batchcode+'&customer_id='+customer_id_en+'&industry_type='+industry_type+'&is_merge='+is_merge+'&city_type='+city_type;

            }else if(paytype_obj.pay_type =='yeepay'){
                ...
                url=protocol_http_host+paytype_obj.pay_url.h5+"?order_id="+order_batchcode+"&customer_id="+customer_id_en+'&industry_type='+industry_type+'&pay_batchcode='+pay_code+'&is_merge='+is_merge+'&city_type='+city_type;

            }else if(paytype_obj.pay_type =='jdpay'){
                var pay_code = get_paycode(pay_user_id,industry_type);
                if(pay_code == null){
                    pay_code = order_batchcode;
                }
                url=protocol_http_host+paytype_obj.pay_url.common+"?order_id="+order_batchcode+"&customer_id="+customer_id_en+"&industry_type="+industry_type+'&pay_batchcode='+pay_code+'&is_merge='+is_merge+'&city_type='+city_type;
            }else if(paytype_obj.pay_type =='paypal'){
                var pay_code = get_paycode(pay_user_id,industry_type);
                if(pay_code == null){
                    pay_code = order_batchcode;
                }
                url=protocol_http_host+paytype_obj.pay_url.common+"?order_id="+order_batchcode+"&customer_id="+customer_id_en+'&industry_type='+industry_type+'&pay_batchcode='+pay_code+'&is_merge='+is_merge+'&city_type='+city_type;
            }else if(paytype_obj.pay_type == 'vlifepay'){
                url=protocol_http_host+paytype_obj.pay_url.common+"?order_id="+order_batchcode+"&customer_id="+customer_id_en+'&industry_type='+industry_type+'&is_merge='+is_merge+'&city_type='+city_type;
            }else if(paytype_obj.pay_type == 'xingyebankpay'){
                url=protocol_http_host+paytype_obj.pay_url.common+"?order_id="+order_batchcode+"&customer_id="+customer_id_en+'&industry_type='+industry_type+'&is_merge='+is_merge+'&city_type='+city_type;
            }else if(paytype_obj.pay_type == "IPSpay"){  //环迅快捷支付
                url=protocol_http_host+paytype_obj.pay_url.common+"?order_id="+order_batchcode+"&industry_type="+industry_type+"&from_type=wx&is_merge="+is_merge+'&city_type='+city_type;
            }else if(paytype_obj.pay_type == "IPSweipay"){  //环迅微信支付
                // url="https://thumbpay.e-years.com/psfp-webscan/onlinePay.do";
                url=protocol_http_host+paytype_obj.pay_url.common+"/weixinpl/IPSpay/IPSweipay.php?order_id="+order_batchcode+"&industry_type="+industry_type+'&is_merge='+is_merge+'&city_type='+city_type;
            }else if(paytype_obj.pay_type =='wftpay'){
                if( "wx" == page_port ){
                    url=protocol_http_host+paytype_obj.pay_url.common+"?order_id="+order_batchcode+"&industry_type="+industry_type+'&is_merge='+is_merge+'&city_type='+city_type;
                }else{
                    url=protocol_http_host+paytype_obj.pay_url.h5+"?order_id="+order_batchcode+"&industry_type="+industry_type+'&is_merge='+is_merge+'&city_type='+city_type;
                }

            }else if(paytype_obj.pay_type == 'healthpay'){
                url=protocol_http_host+paytype_obj.pay_url.common+"?order_id="+order_batchcode+"&customer_id="+customer_id_en+'&industry_type='+industry_type+'&is_merge='+is_merge+'&city_type='+city_type;
            }
            else{
                var pay_code = get_paycode(pay_user_id,industry_type);
                ...
                var url = protocol_http_host+"/weixinpl/mshop/orderlist_detail.php?customer_id="+customer_id_en+"&pay_batchcode="+pay_code;       //跳转到订单详情页
            }
            ...
            $.ajax({
                ...
                dataType: 'json',
                type: 'post',
                data:{'customer_id_en':customer_id_en,'industry_type':industry_type,"order_batchcode":order_batchcode,"internal_opt":"get_returnpage","city_type":city_type,"cash_o2o_type":cash_o2o_type,"orderingretail_opt":opt},
                success:function(data){
                    history.replaceState({}, '',protocol_http_host+data.history_return_page);
        ...
        if( num_len == 6 ){
            ...
            $.post("/weixinpl/mshop/pay_password_controller.php?customer_id="+customer_id_en,{action:'correct',user_id:pay_user_id,paypassword:pay_password},function(res){
                ...
                if( res.status == 1 ){
                    ...
                    //toInternalPay(pay_type);
                    loading(100,1);
                    StartPay(pay_type_obj,extra_info_obj);
                }else{
                    error_time++;
                    if( error_time >= 3 ){
    ...
    //支付完成后跳转页面
    function history_replaceState(industry_type) {console.log('第二步');
...
//            } else {
                history.replaceState({}, '', '/weixinpl/mshop/orderlist.php?customer_id' + customer_id_en + '&currtype=1');       //商城:修改历史记录，支付后返回个人中心
//            }
