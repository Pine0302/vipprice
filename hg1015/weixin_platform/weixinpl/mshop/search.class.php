...
switch ($op){
	case 'search':
		...
		$rtn_data = array();		
		$product_data = array();
		$query = "select id,name,default_imgurl,is_supply_id,orgin_price,now_price,vip_price,isvp,vp_score,cashback,cashback_r,show_sell_count,sell_count,is_free_shipping,need_score,back_currency,is_currency,tax_type,privilege_level,is_privilege,propertyids,storenum,is_QR,is_virtual,pro_card_level_id,islimit,is_first_extend from weixin_commonshop_products where isvalid=true and yundian_id<0 and isout=0 and customer_id=".$customer_id." AND ((is_privilege=1 and (privilege_level LIKE '%$user_level%' or privilege_level = '') ".$pl_con." ) or is_privilege=0)  ";
